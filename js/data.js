...
- 登录后可修改昵称和密码
- 勾选"记住密码"可永不过期免登录
- 若没有账号，可在登录页点击 **「📝 注册新账号」** 自行注册（填写用户名+邮箱+密保问题）
- 忘记密码时，点击 **「🔑 找回密码」**，通过邮箱+密保问题验证即可重置
- 密保问题可在登录后在"我的→密保设置"中随时修改
...
- 抗微生物药按教科书分类标准细分为31个子类（青霉素类、头孢1-5代、头霉素类、碳青霉烯类、氨基糖苷类、大环内酯类、林可霉素类、四环素类、喹诺酮类、磺胺类等）
- 每种药品标注使用级别：非限制级、限制级、特殊使用级
- 疾病分类标签：128种疾病详情（含定义、症状、诊断、治疗原则+关联药品+关联指南）
- 分类可折叠，支持搜索

## 四、指南法规（34条 + 8部法规）
- 按系统分组（肿瘤、心血管、神经、呼吸等），每组可折叠
- 每条指南包含完整临床内容（分级、流程、方案、剂量）
- 支持全文搜索，部分指南含原始PDF全文
- 法律法规：收录8部核心药品法律法规全文（药品管理法、处方管理办法、抗菌药物管理办法、麻精药品管理条例、GMP、GSP、疫苗管理法等）

## 五、药品详情与对比
- 完整药品信息（适应症/禁忌/不良反应/用法用量/储存/相互作用/说明书）
- 药品详情显示使用级别（非限制级/限制级/特殊使用级）和高危警示标签
- 个人备注功能（仅自己可见）
- 🧺 **对比篮**：这是本系统特色功能
  - 在首页点击「药品对比」进入
...
## 六、计算工具
- 肌酐清除率CrCl、eGFR估算、Child-Pugh评分
- INR与华法林调整、等效剂量换算、稳态浓度/负荷剂量
- 输液速度与泵速、浓度稀释计算、皮试液配置、医嘱标准化换算
- 儿童年龄公式、体重估算、体表面积BSA
- 抗生素效价换算（万单位↔mg）、抗生素肾功剂量调整（CrCl指导）
- 抗生素DDD/AUD计算（WHO标准）、儿科用量计算、手术切口分类查询
- 化验指标参考值（11大类50+项）
...
## 九、输液配伍（33条）
20种注射药配伍禁忌+10组口服配伍禁忌+输液通用原则（载体选择/速度控制/配伍判断）

## 十、我的
- 昵称修改、密码修改
- 我的收藏（药品/指南/法规均可收藏，支持备注）
- 编辑记录（自己的操作日志）
- 使用帮助、免责声明
