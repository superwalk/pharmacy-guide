...
function openGuide(gid) {
  ...
  document.getElementById('label-content').innerHTML=`
    ...
    <div style="font-size:12px;color:var(--text-light);display:flex;align-items:center;justify-content:space-between"><span><span class="badge badge-blue">${g.system||'法律法规'}</span><span>${g.year||''}</span> ${sourceBadge(g.id, allGuides())}</span><span style="display:flex;align-items:center;gap:4px"><span id="guide-fav-star" style="cursor:pointer;font-size:14px;user-select:none" title="收藏此指南">${isFav(g.id)?'⭐':'☆'}</span>${isEditor()?`<span id="guide-edit-btn" style="color:var(--primary);cursor:pointer;font-size:12px;flex-shrink:0">编辑</span>`:''}</span></div>
...
function renderFavorites() {
  ...
  fv.forEach(function(id){
    var d = findDrug(id);
    if (d) { favItems.push({id:id, type:'drug', name:d.name, cat:d.category}); return; }
    var g = allGuides().find(function(x){return x.id===id;});
    if (g) { favItems.push({id:id, type:'guide', name:g.title, cat:g.system}); return; }
    g = LAWS.find(function(x){return x.id===id;});
    if (g) { favItems.push({id:id, type:'guide', name:g.title, cat:'法律法规'}); return; }
    if (id.indexOf('inf_q_') === 0) {
      favItems.push({id:id, type:'infquery', name:'🔍 输液查询：'+id.slice(6).replace(/_/g, ', '), cat:'配伍禁忌'});
      return;
    }
    if (id.indexOf('calc-')===0 && typeof findCalcTool === 'function') {
      var cid = id.slice(5); var ct = findCalcTool(cid);
  ...
  container.querySelectorAll('#note-content .info').forEach(function(el){ el.onclick=function(){
    ...
    else if (tp==='infquery'){ openInfusionQuery(id); }
...
function openHealthEdu(hid) {
  ...
  document.getElementById('label-content').innerHTML=`
    ...
    <div class="subtitle-row" style="font-size:12px;color:var(--text-light);display:flex;align-items:center;justify-content:space-between"><span>${h.cat} ${sourceBadge(h.id, HEALTH_EDU)}</span><span id="he-fav-star" style="cursor:pointer;font-size:14px;user-select:none" title="收藏此科普">${isFav(h.id)?'⭐':'☆'}</span></div>
...
function renderInfusion() {
  ...
  var listHtml = cats.map(function(cat){
    ...
    if (items.length===0) return '';
    return '<div class="cat-card" style="margin-bottom:8px"><div class="cat-header" style="cursor:pointer" onclick="toggleGuideGroup(this)" data-expanded="false"><span class="cat-name">'+cat+'</span><span style="font-size:12px;color:var(--text-light)">'+items.length+' 条 <span class="guide-arrow">▶</span></span></div><div class="guide-items" style="display:none">'+items.map(function(i){return '<div class="guide-item" data-iid="'+i.id+'" style="display:flex;gap:6px;align-items:center;padding:8px 4px;cursor:pointer;border-bottom:1px solid var(--border);font-size:13px"><span style="width:6px;height:6px;background:#7C3AED;border-radius:3px;flex-shrink:0"></span><span style="color:var(--text-body);flex:1;font-weight:600">'+highlightKw(i.drug, kw)+'</span><span style="font-size:11px;color:var(--text-light)">'+ (i.vehicle||'') +'</span></div>';}).join('')+'</div></div>';
  }).join('');
...
function doInfusionQuery() {
  ...
  var queryTerm = document.getElementById('inf-query-input').value.trim();
  var qId = 'inf_q_' + queryTerm.replace(/[,，\s]+/g, '_');
  var favs = getFavs();
  var isQFav = favs.indexOf(qId) >= 0;
  html += '<div style="display:flex;gap:6px;margin-bottom:6px"><span class="inf-qfav" data-qid="'+qId+'" data-qterm="'+queryTerm.replace(/"/g,'&quot;')+'" style="cursor:pointer;font-size:13px;user-select:none;color:var(--primary)">' + (isQFav ? '⭐ 已收藏此查询' : '☆ 收藏此查询') + '</span></div>';
  ...
  // 绑定收藏查询按钮
  container.querySelectorAll('.inf-qfav').forEach(function(btn){
    btn.onclick = function(e){
      e.stopPropagation();
      var qid = btn.getAttribute('data-qid');
      var qterm = btn.getAttribute('data-qterm');
      if (!qid) return;
      var favs = getFavs();
      if (favs.indexOf(qid) >= 0) { favs = favs.filter(function(x){return x!==qid;}); }
      else { favs.push(qid); }
      localStorage.setItem(favKey(), JSON.stringify(favs));
      trySync('favorites', { username: currentUser ? currentUser.username : '', content_id: qid, list: JSON.stringify(favs) });
      doInfusionQuery();
    };
  });
  // 绑定配伍查询工具收藏
...
function openInfusion(iid) {
  ...
  document.getElementById('label-content').innerHTML=`
    ...
    <div style="font-size:13px;color:var(--text-light);margin-bottom:12px;display:flex;align-items:center;justify-content:space-between"><span><span class="badge badge-blue">${i.cat}</span> ${sourceBadge(i.id, INFUSION_DATA)}</span>${isEditor()?`<span id="inf-edit-btn" style="color:var(--primary);cursor:pointer;font-size:12px;flex-shrink:0">编辑</span>`:''}</div>
...
function openMedEdu(mid){
  ...
  pushScreen('label');
  document.getElementById('label-content').innerHTML='<div class="section-title" style="font-size:22px">'+m.drug+'</div><div class="subtitle-row" style="font-size:12px;color:var(--text-light);display:flex;align-items:center;justify-content:space-between;margin-bottom:12px"><span class="badge badge-blue">'+m.cat+'</span> '+sourceBadge(m.id, MED_EDU)+'<span id="me-fav-star" style="cursor:pointer;font-size:14px;user-select:none" title="收藏此用药教育">'+(isFav(m.id)?'⭐':'☆')+'</span></div><div id="mededu-body"><div style="text-align:center;padding:20px;color:var(--text-light)">加载中…</div></div>';
...
function showEditBtn(item){ 
  ...
  if((item.type==='edu'||item.type==='med') && isEditor()){
    ...
    if(sub && !sub.querySelector('.inline-edit-btn')){
      ...
      eb.style.cssText='color:var(--primary);cursor:pointer;font-size:12px;flex-shrink:0';
      eb.textContent='编辑';
