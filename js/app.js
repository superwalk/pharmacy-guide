...
function initProfileMenus() {
  ...
  document.getElementById('menu-change-pw').onclick=()=>{ showModal('修改密码','<input id="old-pw" type="password" placeholder="原密码"><input id="new-pw" type="password" placeholder="新密码"><label style="font-size:13px;color:var(--text-light);cursor:pointer;display:flex;align-items:center;gap:4px"><input type="checkbox" id="show-pw" onchange="var t=this.checked?\'text\':\'password\';document.getElementById(\'old-pw\').type=t;document.getElementById(\'new-pw\').type=t"> 显示密码</label>',[{label:'取消'},{label:'确认修改',primary:true,onClick:()=>{ const o=document.getElementById('old-pw').value; const n=document.getElementById('new-pw').value; if(!n||n.length<4){ toast('密码至少4位'); return; } const r=changePassword(o,n); if(!r.ok) toast(r.msg); else toast('密码已修改，下次登录生效'); }}]); };
